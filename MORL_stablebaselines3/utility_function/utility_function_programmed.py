...
class Utility_Function(nn.Module):
    def __init__(self, reward_shape=2, norm=True, lamda=0.1, function_choice= 0):
        super().__init__()
...
class Utility_Function_Programmed(nn.Module):
    def __init__(self, reward_shape=3, norm=True, lamda=0.1, function_choice=0, keep_scale=True):
        super().__init__()
        self.function_choice = function_choice
        ...
        # 1) 수익 중심 (r0 위주)
        self.utility_functions.append(lambda x: 0.7 * x[:, 0] + 0.15 * x[:, 1] + 0.15 * x[:, 2])

        # 2) 수익 vs 비용 trade-off (비용 강하게 패널티)
        self.utility_functions.append(lambda x: 0.6 * x[:, 0] + 0.3 * x[:, 1] + 0.1 * x[:, 2])

        # 3) 비용과 리스크(1,2번)를 더 중요하게 보는 경우
        self.utility_functions.append(lambda x: 0.4 * x[:, 0] + 0.3 * x[:, 1] + 0.3 * x[:, 2])
